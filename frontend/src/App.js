...
function App() {
  ...
  return (
    <div className="App">
      ...
      <div className='menu-details'><p>Thank you<br/>for<br/>visiting<br/>MILKWALL</p></div>
      <form onSubmit={e => {
        e.preventDefault();
        setIsEditing(false);
      }}>
        {cannotEditMessage && <p className='cannot-edit' onClick={() => setCannotEditMessage(false)}>Try again soon. Ten minutes must pass between visitor edits. <span className='close-x'>ˣ</span></p>}
     <label>
        {isEditing ? (
          <textarea
          rows='6'
      ...
      {isEditing && <button type="submit" onClick={editHandler}>Post</button> }
    </form>
    </div>
  );
}
